//appModule.controller('Product_CreateOrEditController', ['$scope', '$rootScope', '$route', '$routeParams', '$location', '$http', 'CollectionService', 'DataContext', 'codeTrimRoot', function ($scope, $rootScope, $route, $routeParams, $location, $http, collectionService, dataContext, codeTrimRoot) {
//DatePickers
$scope.dtOpened = {};
...
repoForLookups = dataContext.repository('ProductCategories');
repoForLookups.getAll({
    pageSize: 200
}).success(function (data) {
    $scope.$$lookups.CategoryId = data.items;
});
...
if (queryStringParams.id) {
    queryStringParams.includes = ['Category', 'OrderDetails'];
...
$scope.actions = {
    save: function () {
        ...
        if ($scope.Product.Id > 0) return $scope.actions.update();
        else return $scope.actions.create();
    },
    update: function () {
        var json = JSON.stringify($scope.Product);
